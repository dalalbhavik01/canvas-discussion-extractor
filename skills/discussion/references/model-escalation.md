...
4. Consider only permitted actions. Prefer a fresh observation, a targeted read-only retry, replay from verified local records, or a local serialization repair. A repair preserves the approved method; changing extraction source or introducing a new transport requires the workflow's capability gate and prior agreement. Desktop text-editor/clipboard relays are not automatic serialization repairs. Never propose an edit to Canvas, fabricated entry, changed expected count, silent reply truncation, or cross-cohort substitution as a repair.
